...
describe('featureReducersReducer', () => {
  ...
  it('calls feature reducers in order', () => {
    const r1 = sinon.spy((state, action) =>
      action.type[0] === 'a' ? { ...state, a: 1 } : state
    )
    const r2 = sinon.spy((state, action) =>
      action.type[1] === 'b' ? { ...state, b: 2 } : state
    )
    const r3 = sinon.spy((state, action) =>
      action.type[2] === 'c' ? { ...state, c: 3 } : state
    )
    ...
    expect(reducer(initState, { type: 'abc' })).to.deep.equal({
      ...initState,
      a: 1,
      b: 2,
      c: 3,

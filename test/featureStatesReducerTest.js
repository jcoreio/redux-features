...
describe('featureStatesReducer', () => {
  ...
  it('handles addFeature correctly', () => {
    ...
    expect(reducer({ a: 'LOADING' }, addFeature('a', {}))).to.deep.equal({
      a: 'LOADING',
    })
    expect(reducer({ a: 'LOADED' }, addFeature('a', {}))).to.deep.equal({
      a: 'LOADED',
    })
  })
  it('handles loadFeature correctly', () => {
    expect(reducer({}, loadFeature('a', {}))).to.deep.equal({})
    expect(reducer({ a: 'NOT_LOADED' }, loadFeature('a', {}))).to.deep.equal({
      a: 'LOADING',
    })
    expect(
      reducer({ a: new Error('test') }, loadFeature('a', {}))
    ).to.deep.equal({ a: 'LOADING' })
    expect(reducer({ a: 'LOADED' }, loadFeature('a', {}))).to.deep.equal({
      a: 'LOADED',
    })
  })
  it('handles installFeature correctly', () => {
    expect(reducer({}, installFeature('a', {}))).to.deep.equal({})
    expect(reducer({ a: 'NOT_LOADED' }, installFeature('a', {}))).to.deep.equal(
      { a: 'LOADED' }
    )
    expect(reducer({ a: 'LOADING' }, installFeature('a', {}))).to.deep.equal({
      a: 'LOADED',
    })
  })
  it('handles replaceFeature correctly', () => {
    expect(reducer({}, replaceFeature('a', {}))).to.deep.equal({})
    expect(reducer({ a: 'NOT_LOADED' }, replaceFeature('a', {}))).to.deep.equal(
      { a: 'NOT_LOADED' }
    )
    expect(reducer({ a: 'LOADED' }, replaceFeature('a', {}))).to.deep.equal({
      a: 'NOT_LOADED',
    })
  })
  it('handles setFeatureState correctly', () => {
    ...
    expect(
      reducer({ a: 'NOT_LOADED' }, setFeatureState('a', 'LOADING'))
    ).to.deep.equal({ a: 'LOADING' })
    expect(
      reducer({ a: 'LOADED' }, setFeatureState('a', 'LOADING'))
    ).to.deep.equal({ a: 'LOADING' })
  })
  it('handles loadInitialFeatures correctly', () => {
    ...
    expect(
      reducer(
        { a: 'NOT_LOADED', b: 'LOADING', c: 'LOADED' },
        loadInitialFeatures()
      )
    ).to.deep.equal({
      a: 'NOT_LOADED',

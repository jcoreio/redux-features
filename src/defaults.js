...
export const defaultCreateReducer: (<S, A: { type: $Subtype<string> }>(
  initialState: S,
  reducers: { [actionType: string]: Reducer<S, A> }
) => Reducer<S, A>) &
  (<S, A: { type: $Subtype<string> }>(reducers: {
    [actionType: string]: Reducer<S, A>,
  }) => Reducer<S, A>) = function <S, A: { type: $Subtype<string> }>(): Reducer<
  S,
  A
> {
...
export function defaultComposeReducers<S, A>(
...
export function defaultCreateMiddleware<
  S,
  A: { type: $Subtype<string> }
>(middlewares: { [actionType: string]: Middleware<S, A> }): Middleware<S, A> {
...
export function defaultComposeMiddleware<S, A: { type: $Subtype<string> }>(
  ...middlewares: Array<Middleware<S, A>>
): Middleware<S, A> {
  return (store) => (next) =>
    middlewares.reduceRight((next, handler) => handler(store)(next), next)
}
